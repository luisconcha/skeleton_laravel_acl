<?php
/**
 * Created by PhpStorm.
 * User: Luis Alberto Concha Curay
 * Date: 19/12/18
 * Time: 23:23
 */

return [

    'login'                    => 'Login',
    'logout'                   => 'Logout',
    'lang'                     => 'pt-br',
    'register'                 => 'Register',
    'name'                     => 'Name',
    'email'                    => 'E-Mail Address',
    'password'                 => 'Password',
    'confirmation_password'    => 'Confirm Password',
    'remember_me'              => 'Remember Me',
    'forgot_your_password'     => 'Forgot Your Password',
    'reset_password'           => 'Reset Password',
    'send_password_reset_link' => 'Send Password Reset Link',
    'change_language'          => 'Change language',

    'home'                 => 'Home',
    'panel'                => 'Panel',
    'dashboard'            => 'Dashboard',
    'actions'              => 'Actions',
    'list'                 => ':page list',
    'new_record'           => 'Add',
    'user_list'            => 'User',
    'user'                 => 'user',
    'create_crud'          => 'New :page',
    'edit_crud'            => 'Edit :page',
    'show_crud'            => ':page detail',
    'search'               => 'Search',
    'clear'                => 'Clear',
    'add'                  => 'Save',
    'edit'                 => 'Edit',
    'edit_permissions'     => 'Edit Permissions',
    'delete'               => 'Delete',
    'description'          => 'Description',
    'permission_list'      => 'Permissions',
    'permission'           => 'Permission',
    'resource_description' => 'Resource Description',
    'create_or_edit'       => 'Create or edit',
    'role_list'            => 'Roles',
    'role'                 => 'Role',
    'not_authorized'       => 'You\'are not authorized to access this feature.',
    'system_menus'         => 'System menus',

    'record_added_successfully'           => 'Record added successfully!',
    'record_adding_record'                => 'Error adding record!',
    'successfully_edited_record'          => 'Successfully edited record!',
    'error_while_changing_register'       => 'Error while changing registry!',
    'delete_this_record'                  => 'Delete this record?',
    'registration_deleted_successfully'   => 'Registration deleted successfully!',
    'error_deleting_the_registry'         => 'Error deleting the registry!',
    'impossible_delete_the_record'        => 'Impossible to delete the record:',
    'registration_related_to_another'     => 'It is not possible to delete the paper :role_name because it is related to other records',
    'unable_to_delete_administrator_role' => 'It is impossible to delete the role of :role_name',
    'permissions_assigned_successfully'   => 'Permissions assigned successfully.',
    'unable_assign_permissions'           => 'Unable to assign permissions.',
    'can_not_auto_delete'                 => 'Can not auto-delete.',
    'sign_in_to_start_your_session'       => 'Sign in to start your session.',
    'i_agree_to_the_terms'                => 'I agree to the terms.',
    'i_already_have_a_membership'         => 'I already have a membership.',
    'there_are_no_records'                => 'There are no records.',
    'resource_name'                       => 'Resource name.',


    'language_pt' => 'Portuguese',
    'language_es' => 'Spanish',
    'language_en' => 'English',

];